...
function App() {
  ...
  const [tiemposDes, setTiemposDes] = useState([]) // Ahora es un array
  ...
  const avanzarFase = () => {
    ...
    // 1. Lógica de transición de ACTIVIDAD a DESCANSO
    if (modo === 'actividad') {
      // Obtenemos el tiempo de descanso correspondiente (si no hay suficientes, usamos el último ingresado)
      const tDescanso = tiemposDes[indiceActual] || tiemposDes[tiemposDes.length - 1] || 0;
    ...
    // 2. Lógica para pasar al SIGUIENTE EJERCICIO o RONDA
          ...
          {confirmandoSalida ? (
            <div className="menu-confirmacion">
              ...
              <div className="botones-accion">
                ...
                <button className="btn-control btn-resume" onClick={() => setConfirmandoSalida(false)}>VOLVER</button>
              </div>
            </div>
          ) : (
            <>
              <h1 className="etiqueta-estado">{pausado ? 'PAUSA' : modo.toUpperCase()}</h1>
              <div className="reloj-digital">{formatearTiempo(segundosRestantes)}</div>
              <div className="stats-contenedor">
                <div className="stat-item"><span>RONDA</span><p>{rondaActual}/{repeticiones}</p></div>
                <div className="stat-item"><span>EJERCICIO</span><p>{indiceActual + 1}/{tiempos.length}</p></div>
              </div>
              <div className="botones-accion">
                <button className={`btn-control ${pausado ? 'btn-resume' : 'btn-pause'}`} onClick={() => setPausado(!pausado)}>
                  {pausado ? 'REANUDAR' : 'PAUSAR'}
                </button>
                <button className="btn-control btn-stop" onClick={() => setConfirmandoSalida(true)}>FINALIZAR</button>
              </div>
            </>
          )}
        </div>
    ...
    return (
      <div className="pantalla-final">
        <div className="card-final">
          ...
          <button className="btn-control btn-resume" onClick={iniciarRutina}>REPETIR</button>
          <button className="btn-control btn-stop" onClick={confirmarFinalizar}>INICIO</button>
        </div>
      </div>
    )
  }
        ...
        <form onSubmit={iniciarRutina}>
          <div className="campo">
            ...
            <input 
              ...
              required 
              onChange={(e) => setTiempos(e.target.value.split(',').map(t => Number(t.trim())).filter(t => t > 0))} 
            />
          </div>
          <div className="campo">
            <label>Tiempos de descanso (s) <br />(poner varios solo si se activa la opcion Descanso entre ejercicios):</label>
            <input 
              type="text" 
              placeholder="Ej: 10, 15, 10" 
              onChange={(e) => setTiemposDes(e.target.value.split(',').map(t => Number(t.trim())).filter(t => t > 0))} 
            />
          </div>
          <div className="campo-checkbox">
            <input type="checkbox" id="descansoInt" checked={descansoIntermedio} onChange={(e) => setDescansoIntermedio(e.target.checked)} />
